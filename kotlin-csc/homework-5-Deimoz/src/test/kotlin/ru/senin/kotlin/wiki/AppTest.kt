...
class AppTest {
    ...
    @Test
    @Timeout(TIMEOUT)
    fun `multiple big XML with multiple threads`() {
        testInputs("big.xml,big.xml,big.xml,big.xml", threads = 4)
    }

    @ExperimentalTime
    @Test
    @Timeout(TIMEOUT)
    fun `not well formed XML with correct xml`() {
        testInputs("not-well-formed.xml,simple.xml", threads = 2)
    }
